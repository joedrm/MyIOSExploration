...
- (CAShapeLayer*) buildEmitterSparkle:(CGSize)viewBounds text:(NSString*)text startTime:(NSTimeInterval)startTime
{
    ...
    UIBezierPath *path = [self createPathForText:text fontHeight:height fontName:CFSTR("Helvetica")];
...
#pragma mark - buildAnimatedScrollText
...
{
    CGFloat height = viewBounds.height/10;
    UIBezierPath *path = [self createPathForText:text fontHeight:height fontName:CFSTR("Noteworthy-Bold")];
    ...
    textLayer.strokeColor = BrightBlue.CGColor;
    if ((BOOL)(arc4random() % 2))
    {
        textLayer.fillColor = [[UIColor blueColor] CGColor];
    }
    else
    {
...
{
    CAShapeLayer *maskLayer = [self createMaskHoleLayer:viewBounds startTime:startTime];
...
- (CALayer*) buildAnimatedPhotoLinearScroll:(CGSize)viewBounds photos:(NSMutableArray*)photos startTime:(NSTimeInterval)startTime
{
    ...
    {
        UIImage *image = [self getThumbnailImageByALAsset:asset];
        if (image)
        {
...
- (CALayer*) createPhotoLinearScrollLayer:(CGSize)viewBounds image:(UIImage *)image startTime:(NSTimeInterval)timeInterval
{
    ...
    if (!iOS5 && !iOS6)
    {
        interval = shakeAnimation.beginTime+(shakeAnimation.duration*shakeAnimation.repeatCount);
    }
    CABasicAnimation *moveOutAnimation = [CABasicAnimation animationWithKeyPath:@"position"];
    [moveOutAnimation setFromValue:[NSValue valueWithCGPoint:middlePoint]];
    [moveOutAnimation setToValue:[NSValue valueWithCGPoint:endPointIn]];
...
#pragma mark - buildAnimatedPhotoCentringShow
- (CALayer*) buildAnimatedPhotoCentringShow:(CGSize)viewBounds photos:(NSMutableArray*)photos startTime:(NSTimeInterval)startTime
{
    ...
    for (ALAsset *asset in photos)
    {
        UIImage *image = [self getCropImage:[self getFullResolutionImageByALAsset:asset] videoSize:viewBounds];
        if (image == nil)
        {
            NSLog(@"getFullResolutionImage's crop is failed.");
            image = [self getThumbnailImageByALAsset:asset];
        }
...
- (UIImage*) getFullResolutionImageByALAsset:(ALAsset*)asset
{
...
- (CALayer*) BuildVideoBorderImage:(CGSize)viewBounds borderImage:(NSString*)borderImage position:(CGPoint)position
{
    ...
    CALayer *layerImage = [CALayer layer];
    UIImage *image = [self scaleFromImage:[UIImage imageNamed:borderImage] toSize:viewBounds];
    layerImage.contents = (id)image.CGImage;
    layerImage.frame = CGRectMake(0, 0, viewBounds.width, viewBounds.height);
    layerImage.opacity = 0.9;
    layerImage.position = position;
...
#pragma mark - BuildAnimationPhotoDrop
- (CALayer*) buildAnimatedPhotoDrop:(CGSize)viewBounds photos:(NSMutableArray*)photos startTime:(NSTimeInterval)startTime
{
    ...
    CGFloat width = [self getThumbnailImageByALAsset:photos[0]].size.width;
...
#pragma mark - BuildAnimationPhotoParabola
...
- (CALayer*) createPhotoParabolaLayer:(CGSize)viewBounds image:(UIImage *)image startTime:(NSTimeInterval)timeInterval xAxis:(CGFloat)xAxis
{
    ...
    // Add curled border
    UIImage *curledImage = [[CurledViewBase sharedInstance] setImage:image forLayer:layer];
...
#pragma mark - BuildAnimationPhotoFlare
...
#pragma mark - BuildAnimationPhotoEmitter
- (CALayer*) BuildAnimationPhotoEmitter:(CGSize)viewBounds photos:(NSMutableArray*)photos startTime:(NSTimeInterval)startTime
{
    ...
    CGSize size = [self getThumbnailImageByALAsset:photos[0]].size;
...
- (CALayer*) createPhotoEmitterLayer:(CGSize)viewBounds image:(UIImage *)image startTime:(NSTimeInterval)timeInterval xAxis:(CGFloat)xAxis yAxis:(CGFloat)yAxis
{
    ...
    // Add curled border
    [[CurledViewBase sharedInstance] setImage:image forLayer:layer];
...
#pragma mark - buildAnimatedPhotoExplode
...
#pragma mark - buildAnimatedPhotoExplodeDrop
...
- (CALayer*) createPhotoExplodeDropLayer:(CGSize)viewBounds image:(UIImage*)image  startTime:(NSTimeInterval)timeInterval xAxis:(CGFloat)xAxis
{
    ...
    for (int i=0; i<x; ++i)
    {
        for (int j=0; j<y; ++j)
        {
            ...
            rotateInAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
...
- (UIBezierPath *)pathForLayer:(CALayer *)layer parentRect:(CGRect)rect
{
    ...
    CGFloat layerYPosAndHeight = ((layer.position.y+layer.frame.size.height))*randomFloat() + arc4random()%(int)layer.frame.size.height/4;
...
#pragma mark - buildAnimatedPhotoCloud
...
#pragma mark - buildAnimatedPhotoSpin360
- (CALayer*) buildAnimatedPhotoSpin360:(CGSize)viewBounds photos:(NSMutableArray*)photos startTime:(NSTimeInterval)startTime
{
    ...
    CGSize sizeImage = [self getThumbnailImageByALAsset:photos[0]].size;
    CGPoint position = CGPointZero;
    int gap = 10;
    NSTimeInterval timeInterval = startTime;
    BOOL style = (BOOL)(arc4random() % 2);
    for (int i = 0; i<4; ++i)
    {
        switch (i)
        {
...
- (CALayer*) createPhotoSpin360Layer:(CGSize)viewBounds photos:(NSMutableArray*)photos  startTime:(NSTimeInterval)timeInterval position:(CGPoint)position
{
    ...
    layerImage.borderColor = LightBlue.CGColor;
...
#pragma mark - buildAnimatedPhotoCarousel
...
- (CALayer*)createPhotoCarouselLayer:(CGSize)viewBounds image:(UIImage*)image startTime:(NSTimeInterval)timeInterval xAxis:(CGFloat)xAxis curWhich:(int)curWhich
{
    ...
    CATransform3D finalTransform = [self tranformToBox:1 + curWhich];
